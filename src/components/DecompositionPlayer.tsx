...
// and fight itself.
//
// Unlike the app's other floating panels (CanvasToolbar's popovers, ...),
// this component doesn't position itself with `absolute` — that's left to
  ...
  return (
    <div
      className={`rounded-xl border border-line bg-white/[0.03] px-4 py-3
                  flex flex-col gap-3 font-sans w-[min(92vw,360px)] ${className ?? ''}`}
    >
      <p className="text-ink-faint uppercase tracking-widest text-[10px]">{sequence.title}</p>
      <p className="text-ink-dim text-sm min-h-[2.5em]">{stepLabel}</p>
        ...
        {stops.map((_, i) => (
          <span
            ...
            className={
              ...
              (i <= stepIndex ? 'bg-accent' : 'bg-line-strong')
            }
          />
        ))}
      </div>
